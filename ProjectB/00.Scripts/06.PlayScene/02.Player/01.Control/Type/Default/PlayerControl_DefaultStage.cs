...
public class PlayerControl_DefaultStage : PlayerControl_GamePlay
{
  ...
  //      raycast.OnDashAttackCheckHit -= HandleOnDashAttackCheckHit;
    ...
    protected void HandleOnAttackRangeHit(Collider[] cols)
    {
    ...
    protected void HandleOnAttackHit(Collider[] cols)
    {
        if (GetAttack<PlayerAttack>().isUseSkill == true)
            return;

        PlayerMove_DefaultStage move = GetMove<PlayerMove_DefaultStage>();
        PlayerAttack_GamePlay attack = GetAttack<PlayerAttack_GamePlay>();

        int index = Logic.GetNearestObjectIndex(transform.position, cols, "Enemy", 6);
    ...
    protected void HandleOnDashToTarget(Collider[] cols)
    {
        ...
        if (cols.Length > 0)
        {
            ...
            move.RunToTargetUpdate(cols[index]);
        }
        else
        {
    ...
    private void CheckFirstMove()
    {
        ...
        TimerBuffer firstMovecheck = new TimerBuffer(0.5f);
