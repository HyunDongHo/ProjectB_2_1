...
namespace BackendData.Chart.StagePass
{
    //===============================================================
    ...
    public class Manager : Base.Chart
    {
        ...
        public Dictionary<double, Item> GetChartStagePassData()
        {
            Dictionary<double, Item> StagePassData = new();

            foreach (var data in _dictionary)
            {
                StagePassData.Add(data.Value.Level, data.Value);
            }
            return StagePassData;
        }
    }
}