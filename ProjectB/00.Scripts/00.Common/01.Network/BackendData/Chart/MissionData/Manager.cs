    ...
    public class Manager : Base.Chart {
        ...
        protected override void LoadChartDataTemplate(JsonData json) {
            foreach (JsonData eachItem in json) {
                ...
                _dictionary.Add(info.MissionID, info);
            }
        }
        public Dictionary<int, Item> GetChartMissionData(MissionType missionType)
        {
            Dictionary<int, Item> MissionData = new();

            foreach(var data in _dictionary)
            {
                if (data.Value.MissionType == missionType)
                    MissionData.Add(data.Value.MissionID, data.Value);
            }
            return MissionData;
        }
    }
