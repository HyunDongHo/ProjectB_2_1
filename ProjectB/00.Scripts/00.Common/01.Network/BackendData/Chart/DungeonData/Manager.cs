...
namespace BackendData.Chart.DungeonData
{
    ...
    public class Manager : Base.Chart {
        ...
        public Item GetItem(DungeonType dungeonType, RewardItemType rewardItemType )
        {
            for (int i = 0; i < List.Count; ++i)
            {
                if (List[i].DungeonType == dungeonType && List[i].RewardItemType == rewardItemType)
                {
                    return List[i];
                }
            }
